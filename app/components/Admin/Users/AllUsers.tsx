        ...
        return (
          <>
            <a href={`mailto:${params.row.email}`}        >
              <AiOutlineMail
                className="dark:text-white text-black mt-4 ml-3"
                size={20}
              />
            </a>
          </>
          ...
          <Box m="20px">
            {
              isTeam && (
                <div className="w-full flex justify-end">
                  <div className={`${style.button} !w-[220px]`} onClick={() => setActive(!active)}>
                    Add new Member
                  </div>
                </div>
              )
              ...
              active && (
                ...
                >
                  <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[450px] bg-white dark:bg-slate-900 rounded-[80px] shadow p-4 outline-none">
                    ...
                    <div className="mt-4">
                      <input type="email" placeholder="Enter Email..." className={`${style.input}`} value={email} onChange={(e) => setEmail(e.target.value)} />
                      <select name="" id="" className={`${style.input} !mt-6`}>
                        <option value="admin">Admin</option>
                        <option value="user">User</option>
                      </select>
                      <br />
                      <div className={`${style.button} my-6 !h-[30px]`} onClick={handleSubmit}>
                        Submit
                      </div>
                    </div>
                  </Box>
...

            {
              open && (
                ...
                >
                  <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[450px] bg-white dark:bg-slate-900 rounded-[80px] shadow p-4 outline-none">
                    <h1 className={`${style.title}`}>Are you want to delete this user</h1>
