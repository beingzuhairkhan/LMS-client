...
const categories = [
  ...
  { title: 'CBSE/State Board', description: 'Courses for various educational boards.' },
  { title: 'JEE/NEET/CET Competitive Exams', description: 'Preparation for various competitive exams.' },
  { title: 'Under Gradute (Science)', description: 'BSc Physics , Chemistry , Biology , Mathematics , Information Technology (IT) , Computer Science and Microbiology courses available.' },
  { title: 'Under Gradute  (Commerce)', description: 'Bachelor of Commerce (B.Com) ,Bachelor of Commerce in Banking & Insurance (B.B.I) ,Bachelor of Commerce in Accounting & Finance (B.A.F) ,Bachelor of Management Studies (B.M.S) courses available. ' },
  { title: 'Post Gradute (Commerce)', description: 'Master of Commerce (M.Com in Accountancy) , Master of Commerce in banking & Insurance (M.Com in Management) courses available.' },
];

const About2: React.FC = () => {
  return (
    <div className="p-6 w-full">
      <h1 className="text-3xl font-bold text-center mb-6 dark:text-white text-black">What <span className="text-gradient"  >GARUDA</span> Does?</h1>
      <p className="text-center mb-8 dark:text-white text-black">
        GARUDA institute is a one-to-one platform that provides home and online tuition services and connects teachers to academic institutions. Our platform offers a comprehensive range of academic solutions tailored to meet the specific needs of students from pre-primary to 12th grade, in different streams like CBSE, IB, ICSE, and IGCSE boards. So why search for tuition near me? Sign up today and take the first step toward academic excellence!
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {categories.map((category, index) => (
          ...
          >
            ...
            <p className="text-[#37a39a] mb-4">{category.description}</p>
          </div>
        ))}
      </div>
      <hr className="mt-4" />
